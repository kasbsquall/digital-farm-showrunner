...
A **63-test pytest suite** lives under [`backend/tests/`](backend/tests/) and runs fully offline:
...
- **Character consistency by construction — and now *enforced*.** Keyframe → i2v pins the cast to an established look ([§5a](#5-the-two-signature-techniques)); competitors doing blind text-to-video can't hold a mascot across a daily series. An optional identity-lock gate (`IDENTITY_CHECK`) scores each keyframe against its canonical portrait with `qwen3-vl-plus` (calibrated **0.8–0.9** for a matching character, **0.0** for a mismatch) and **rejects** any take below `IDENTITY_MIN`, regenerating a fresh keyframe — so consistency is a number the pipeline acts on, not just reports.
- **A self-correcting QA gate that never publishes unwatched.** The vision-grounded retake loop is committed and **proven on real Alibaba Cloud generation** (`snapshot.json`): a real 3-attempt run ("Sheep Drops Hay on Pig") regenerated twice on real, vision-grounded `qa_notes`, held the character on-model across all three takes (consistency **0.8**), metered **$0.84** blended, and — never satisfying QA — published the best take as a **draft** rather than going live. Two real episodes on the same infra *did* clear QA (approved, consistency 0.8–0.9). Unsupervised daily publishing is only safe because a step actually watches the footage — and when vision can't watch the clip at all, QA rejects rather than approving blind. (A mock-mode "Wobbly Gold Spheres…" `rejected→rejected→approved` episode is also seeded to illustrate the *approved-after-retries* arc shape.)
- **A per-channel data flywheel (the compounding advantage).** Cast and episodes carry a `channel_id`, and `_load_context` scopes each channel's recent events **and** its top-voted "audience favorites" to that channel — so one creator's audience biases only that creator's writing, and continuity compounds per tenant (not one global pool). Voting is idempotent per `(episode, voter)` via a `votes` table with a unique constraint, so the signal that steers generation can't be inflated by re-posting (dedup, not full Sybil-resistance — a rotating-identity attacker would still need real auth/rate-limiting to defeat) — an advantage a one-off clip tool can't accumulate.
...
- **Scheduling & auto-publish — ✅ shipped and demonstrated.** An unattended daily-channel loop (`backend/scheduler.py`, `SCHEDULER_ENABLED`) runs the showrunner on an interval and publishes on the QA verdict — in-process on app startup, or standalone (`python -m scheduler [--once]`, cron-friendly). **Proof:** a real unattended run produced **3 episodes with no per-episode human trigger** — 2 auto-published (QA approved, ~$0.30 each) and 1 held back as a **draft** (QA rejected after a real retake) — evidence (timestamps, receipts, verdicts) in [`docs/deploy_proof/scheduler_run.json`](docs/deploy_proof/scheduler_run.json). The vision-grounded QA loop is what makes unsupervised publishing safe. Next: even longer multi-beat story arcs.
